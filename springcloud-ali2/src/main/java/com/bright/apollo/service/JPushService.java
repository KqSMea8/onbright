...
public class JPushService {
		...
		if (StringUtils.isEmpty(url)) {
			return PushPayload.newBuilder().setPlatform(Platform.all())
					...
					.setNotification(Notification.newBuilder().setAlert(alert)
							...
							.addPlatformNotification(IosNotification.newBuilder().build()).build())
					.build();
		}
		return PushPayload.newBuilder().setPlatform(Platform.all())
				.setAudience(Audience.newBuilder().addAudienceTarget(AudienceTarget.tag(tag)).build())
				.setNotification(Notification.newBuilder().setAlert(alert)
						.addPlatformNotification(AndroidNotification.newBuilder().addExtra("url", url).build())
						.addPlatformNotification(IosNotification.newBuilder().addExtra("url", url).build()).build())
				.build();
	...
	public static PushPayload buildPushObjectMessageWithExtras(String content, String tag) {
		return PushPayload.newBuilder().setPlatform(Platform.all())
				.setAudience(Audience.newBuilder().addAudienceTarget(AudienceTarget.tag(tag))
						// .addAudienceTarget(AudienceTarget.alias("alias1",
						// "alias2"))
						.build())
				.setMessage(Message.newBuilder()
						// .setMessageContent(content)
						.setMsgContent(content).addExtra("from", "apollo-jpush").build())
				.build();
	}
