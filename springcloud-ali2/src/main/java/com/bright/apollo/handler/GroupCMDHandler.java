...
public class GroupCMDHandler extends BasicHandler{
	...
	public Message<String> process(ClientSession clientSession, Message<String> msg) throws Exception {
		...
		if ("01".equals(isSuccess)) {
			...
			logger.info("---set:" + set + " ---oboxSerialId:" + oboxSerialId
					+ " ---groupAddr:" + groupAddr + " ---nodeAddr:" + nodeAddr);
			TObox obox = oboxService.queryOboxsByOboxSerialId(oboxSerialId);
			if (obox == null) {
				logger.error(String.format("not found %s obox!",
						clientSession.getUid()));
				return null;
			}

			logger.info("---obox:" + new JSONObject().toJSONString(obox));
			TOboxDeviceConfig tOboxDeviceConfig = oboxDeviceConfigService
					.queryOboxConfigByRFAddr(obox.getOboxId(), nodeAddr);
			if (tOboxDeviceConfig == null) {
				return null;
			}

			logger.info("---tOboxDeviceConfig:"
					+ new JSONObject().toJSONString(tOboxDeviceConfig));
			if (set.equals("00")) {
				if (!tOboxDeviceConfig.getGroupAddr().equals("00")
						&& tOboxDeviceConfig.getGroupAddr().equals(groupAddr)) {
					// before
					updateGroupType(oboxSerialId,
			...
			//		logger.info("---tKey:" + new JSONObject().toJSONString(tKey));
					...
					if (tServerGroup != null) {
						logger.info("---tServerGroup:"
								+ new JSONObject().toJSONString(tServerGroup));
						TGroupDevice tGroupDevice = groupDeviceService
								.queryDeviceGroup(tServerGroup.getId(),
										tOboxDeviceConfig.getDeviceSerialId());
						if (tGroupDevice == null) {
							logger.info("---tGroupDevice:"
									+ new JSONObject()
											.toJSONString(tGroupDevice));
