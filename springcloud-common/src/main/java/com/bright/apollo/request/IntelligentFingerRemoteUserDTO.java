...
public class IntelligentFingerRemoteUserDTO implements Serializable{
	...
	public IntelligentFingerRemoteUserDTO(TIntelligentFingerRemoteUser user) {
		...
		if(user.getUserSerialid()<10){
			setPwd("620"+user.getUserSerialid()+(NumberHelper.isNumeric(user.getPwd())?user.getPwd():Base64Util.base64Decrypt(user.getPwd())));
		}else{
			setPwd("62"+user.getUserSerialid()+(NumberHelper.isNumeric(user.getPwd())?user.getPwd():Base64Util.base64Decrypt(user.getPwd())));
		}
